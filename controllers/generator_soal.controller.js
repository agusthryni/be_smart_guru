...
exports.generateCourse = async (req, res) => {
  ...
  const prompt =
    ...
    ` for ${grade} high school grade consisting of ${questions_total} questions and if the question has an equation make them in TeX format the TeX example is look like this \\(x = {-b \\pm \\sqrt{b^2+4ac} \\over 2a}\\) please use the format in example. Each question has ${choices_total} answer choices. The format JSON inside key named data and wrapped in an array with keys for questions, choices, and answers without alphabet to make it easy to parse. An example of the format is like this:
