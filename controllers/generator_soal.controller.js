...
exports.generateCourse = async (req, res) => {
  ...
  const prompt =
    ...
    ` for ${grade} high school grade consisting of ${questions_total} questions and if the question has an equation put them inside this \( \) as example of a question 'Berapakah titik potong parabola y = x^2 dan sumbu x?' you create them like 'Berapakah titik potong parabola \(y = x^2\) dan sumbu x?' and please use this format. Each question has ${choices_total} answer choices. The format JSON inside key named data and wrapped in an array with keys for questions, choices, and answers without alphabet to make it easy to parse. An example of the format is like this:
